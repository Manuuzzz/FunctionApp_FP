param(
    ...
    $AutomationConnection = Get-AutomationConnection -Name $azureRunAsConnectionName
    $null = Add-AzAccount -ServicePrincipal -TenantId $AutomationConnection.TenantId -ApplicationId $AutomationConnection.ApplicationId -CertificateThumbprint $AutomationConnection.CertificateThumbprint
    
    $StorageAccountName = "INSERT SA NAME" 
    $StorageAccountKey = "INSERT KEY // SECURE KEY"
    
    $ContainerName = "INSERT CONTAINER NAME"
    $ArchiveContainerName = "INSERT CONTAINER NAME"
    $BlobPath = "local/"
    ...
    $User = "INSERT USER"
    #$PWord = ConvertTo-SecureString -String (Get-AzKeyVaultSecret -VaultName 'INSERT VAULT NAME'  -Name 'SvcAzureBi').SecretValueText -AsPlainText -Force
    $PWord = ConvertTo-SecureString -String "INSERT USER PASSWORD / SECURE STRING" -AsPlainText -Force
    ...
    Do { 
        $currentResults = Search-UnifiedAuditLog -StartDate $startDate -EndDate $enddate `
                                    -SessionId $sessionName -SessionCommand ReturnLargeSet -ResultSize 1000 -RecordType PowerBIAudit
